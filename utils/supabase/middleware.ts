  ...
  try {
    ...
    if (request.nextUrl.pathname.startsWith('/protected') && guestOnly) {
      return NextResponse.redirect(
        new URL('/sign-in?middleware=sign in required', request.url)
      )
    }

    // Redirect to protected page if user is logged in and tries going to homepage
    if (request.nextUrl.pathname === '/' && authUser) {
      return NextResponse.redirect(
        new URL('/protected?message=testing', request.url)
      )
    }
